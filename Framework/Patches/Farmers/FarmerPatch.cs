...
internal sealed class FarmerPatch() : PatchHelper(typeof(Farmer))
{
    internal void Apply()
    {
    ...
    private static bool StopSittingPrefix(Farmer __instance, bool animate = true)
    {
        ...
        if (furniture.IsSeatHere(__instance.currentLocation))
        {
            ...
            List<Vector2> exit_positions = [];
            Vector2 sit_position = new(furniture.GetSeatBounds().Left, furniture.GetSeatBounds().Top);
            ...
            bounds2.Inflate(1, 1);
            exit_positions.AddRange(Utility.getBorderOfThisRectangle(bounds2));
            foreach (Vector2 exit_position in exit_positions)
            {
                ...
                if (tile_object?.isPassable() == false)
                {
