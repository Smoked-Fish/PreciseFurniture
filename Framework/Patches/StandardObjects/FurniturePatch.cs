...
internal sealed class FurniturePatch() : PatchHelper(typeof(Furniture))
{
    public void Apply()
    {
    ...
    private static bool GetSeatPositionsPrefix(Furniture __instance, ref List<Vector2> __result, bool ignore_offsets = false)
    {
        ...
        Vector2 rectTileLocation = new((float)__instance.boundingBox.X / 64f, (float)__instance.boundingBox.Y / 64f);
        List<Vector2> seat_positions = [];
